...
- Yahoo Finance public chart/search endpoints for `/chart` stock and major-crypto images
...
Smart Intel uses free data. Paid-trend impact improves after the bot has multiple snapshots for a token. Cluster checks use cautious wording and are not proof of wallet relationships. Explain-my-loss uses tracked calls/current market data unless a future wallet-history layer is added. Chart images are generated by the bot and posted directly in Telegram, with optional buttons underneath for live chart/source links.
...
`TICKER_ALIASES` lets you force exact ticker matches before Dexscreener search. It is blank by default.
